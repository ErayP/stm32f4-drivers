...
int main(void)
{
	...
	*/
	...
	while(1){
		...
		/*
		 *
		 *Çünkü mesaj okurken Arduino aynı hattı tekrar oynatırsa (bounce vs) tekrar tekrar “mesaj var!” diye bölmesin.
		 *
		 */
		...
		while(!rcvStop)
		{
			/* burda karakter karakter okuyoruz örneğin 'm' m = 0110 1101   // 0x6D buda 8 bit yani 1 byte bunu okuduktan sonra data registerımız dff'i
			 * 8 bit olarak ayarlandığı için data register doluyor buda RXNE bayrağını bir yapıyor zaten receive data ıt fonksyionunda RXNEIE 1 yaptığımız için
			 * kesmeler aktif oluyor data register dolduğunda kesme geliyor şu fonksiyon çalışıyor
			 *
			 * void SPI_IRQHandling(SPI_Handle_t *pSPIHandle)
			 *
			 * bu fonksiyondan sonra
			 *
			 * static void spi_rxne_interrupt_handle() burası çalışıyor
			 *
			 * sonra burası
			 *
			 * void SPI_ApplicationEventCallback(SPI_Handle_t *pSPIHandle,uint8_t AppEv)
			 *
			 * sonra tekrar dummy yolluyor tüm mesaj bitince çıkıyor
			 *
			 *if(ReadByte == '\0' || ( i == MAX_LEN)){
			 *mesajın sonuna geldiğimizde yani '\0' ı gördüğümüzde while kırılıyor ve çıkış sağlanıyor
			 *  */
